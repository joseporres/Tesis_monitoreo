...
echo "building image legacy-java-application-buildstage"
pushd source
%CONTAINER_RUNTIME% build -f Dockerfile.buildstage -t legacy-java-application-buildstage .
popd

echo "building image legacy-java-application"
pushd source
%CONTAINER_RUNTIME% build -f Dockerfile -t legacy-java-application .
popd
