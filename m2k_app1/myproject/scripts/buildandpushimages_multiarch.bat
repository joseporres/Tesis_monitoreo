...
echo "building and pushing image legacy-java-application-buildstage"
pushd source
docker buildx build --platform ${PLATFORMS} -f Dockerfile.buildstage --push --tag ${REGISTRY_URL}/${REGISTRY_NAMESPACE}/legacy-java-application-buildstage .
popd

echo "building and pushing image legacy-java-application"
pushd source
docker buildx build --platform ${PLATFORMS} -f Dockerfile --push --tag ${REGISTRY_URL}/${REGISTRY_NAMESPACE}/legacy-java-application .
popd
